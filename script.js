...
const imcCalc = () => {
    if(!name.value || !height.value || !weight.value) return result.textContent = 'preencha todos os compos'
    const valorImc = (+weight.value / (+height.value * +height.value)).toFixed();
    console.log (+weight.value ,+height.value , name.value)
    result.textContent = getText(imcCalc);
    result.textContent = `${name.value} voce está ${getText(valorImc)}. com seu IMC em:${valorImc}`;
}
